...
func NewResponse(cataloger catalog.MeshCataloger, proxy *envoy.Proxy, discoveryReq *xds_discovery.DiscoveryRequest, cfg configurator.Configurator, _ certificate.Manager, proxyRegistry *registry.ProxyRegistry) ([]types.Resource, error) {
	...
	key := proxy.GetPodUID()+ "rds"
	r, found := cataloger.GetWitesandCache(key)
	if found {
		return r, nil
	}
	...
	routeConfiguration := route.BuildRouteConfiguration(cataloger, inboundTrafficPolicies, outboundTrafficPolicies, proxy, cfg)
	...
	cataloger.SetWitesandCache(key, rdsResources)
